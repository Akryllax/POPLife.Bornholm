...
_query = switch(_side) do {
	...
	case civilian: {_returnCount = 12; format["SELECT playerid, name, cash, bankacc, adminlevel, donatorlvl, civ_licenses, arrested, civ_gear, reblevel, civPosition, alive FROM players WHERE playerid='%1'",_uid];};
	...
	case east: {_returnCount = 10; format["SELECT playerid, name, cash, bankacc, adminlevel, donatorlvl, civ_licenses, merlevel, mer_gear, blacklist FROM players WHERE playerid='%1'",_uid];};
};
...
switch (_side) do {
	...
	case civilian: {
		...
		//Position: Cogemos el valor de la posicion. Si es NULL, engañamos y ponemos [0,0,0] en su lugar.
		_new = [(_queryResult select 10)] call DB_fnc_mresToArray;
		if(typeName _new == "STRING") then {_new = call compile format["%1", _new];};
		if(count _new != 3 OR (typeName _new != "ARRAY")) then {_new = [0,0,0];};
		_queryResult set[10,_new];
		
		//Tinyint to BOOLEAN, 'alive'.
