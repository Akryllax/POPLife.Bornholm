...
_price = _price*0.65;
if(life_myfunds < _price) exitWith {hint format["Necesitas %1 para quedarte el vehiculo",_price]; };
...
if(_check) exitWith {hint "Demasiada policia cerca!";};
if(typeOf _vehicle == "B_MRAP_01_F") exitWith {hint "Desafortunadamente este vehiculo no te lo puedes quedar...en todo caso siempre lo podras vender"; };
_color = 0;
...
hint format["Pagastes %1 para quedarte el vehiculo en tu garage.",_price];
