...
if([false,_food,1] call life_fnc_handleInv) then {
	switch (_food) do
	{
		...
		case "tuna": {_val = 50};
		case "lataatun": {_val = 100};
		case "surimi": {_val = 100};
		case "mullet": {_val = 80};
	...
	if(_sum > 100) then {_sum = 100; player setFatigue 1; hint "Has comido demasiado. Durante un tiempo te sentiras cansado.";};
	life_hunger = _sum;
};