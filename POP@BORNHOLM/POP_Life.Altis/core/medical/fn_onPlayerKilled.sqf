...
if(!isNull _killer && {_killer != _unit} && {side _killer != west} && {alive _killer}) then {
	if(vehicle _killer isKindOf "LandVehicle") then {
		[[getPlayerUID _killer,_killer getVariable["realname",name _killer],"1"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
...
if(side _killer == west && playerSide != west) then {
	...
	if(!life_use_atm && {pop_dinero > 0}) then {
		[format[localize "STR_Cop_RobberDead",[pop_dinero] call life_fnc_numberText],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		pop_dinero = 0;
	};
};
