...
if(_val > pop_banco) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > pop_banco) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

pop_banco = pop_banco - (_val + _tax);
