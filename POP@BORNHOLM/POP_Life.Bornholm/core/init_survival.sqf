[] spawn  {
	...
	_fnc_food = {
		if ((profileNamespace getVariable "hambre") < 2) then {player setDamage 1;
	...
	_fnc_water = {
		if ((profileNamespace getVariable "sed") < 2) then {player setDamage 1;
...
[] spawn
{
	...
	private ["_pos", "_posVieja", "_posNueva", "_posbaja"];
	sleep 300;
	while {true} do {
		sleep 180;
		_posbaja  = getpos player;
		...
		if (profilenamespace getvariable "vivo") then {
				if !(_pos) then {
					if (vehicle player != player) then {
							profileNamespace setVariable ["posicion", _posbaja];
						} else {
