...
*/
private["_uid","_side","_query","_return","_queryResult","_qResult","_handler","_thread","_tickTime","_loops","_returnCount","_playerPos","_liveornot"];
...
waitUntil{sleep (random 0.3); !DB_Async_Active};
_tickTime = diag_tickTime;
_liveornot = [_query,2] call DB_fnc_asyncCall;
...
if((EQUAL(EXTDB_SETTINGS("MySQL_Query"),1))) then {
       ["diag_log",[
               ...
               format["QUERY: %1",_query],
               format["Time to complete: %1 (in seconds)",(diag_tickTime - _tickTime)],
               format["Result: %1",_liveornot],
               "------------------------------------------------"
       ]] call TON_fnc_logIt;
};

_playerPos = _liveornot select 1;
_playerPos = call compile format["%1", _playerPos];
_queryResult set[13,_playerPos];

_queryResult set[14,([_liveornot select 0,1] call DB_fnc_bool)];
