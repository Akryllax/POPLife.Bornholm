...
_blacklist = [
"life_fnc_breakleg","db_fnc_asynccall","db_fnc_mrestoarray","life_fnc_welcome","life_fnc_goodbye","life_fnc_newMsg","life_fnc_earthquake",
"life_fnc_getdpmission","life_fnc_interactionmenuopen"
];
...
{!(toLower(_functionName) in _whitelist)} && {!(["AUCTION_fnc_",_functionName] call BIS_fnc_inString)} && {!(["SOCK_fnc_",_functionName] call BIS_fnc_inString)} && {!(["POLICE_fnc_",_functionName] call BIS_fnc_inString)}) exitWith {false};
...
if(_callerUID != "__SERVER__" && _callerName != "__SERVER__" && toLower(_functionName) in ["alukac_fnc_stashReport","alukac_fnc_broadcastAlert","life_fnc_netsetvar","life_fnc_stripdownplayer","life_fnc_loadgear","life_fnc_spawnindicator"]) then {
	...
	//NOPE.
	if(toLower(_functionName) in ["life_fnc_spawnindicator","life_fnc_netsetvar","life_fnc_stripdownplayer","life_fnc_loadgear"]) exitWith {
		_exitScope = true;
		[_callerName,_callerUID,format["MPEXEC:CMD:%1:ARGS:%2",_functionName,str(_params)]] call alukac_fnc_stashReport;
	};
