...
if(_val > ALUK_bankedwonga) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > ALUK_bankedwonga) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

ALUK_bankedwonga = ALUK_bankedwonga - (_val + _tax);
