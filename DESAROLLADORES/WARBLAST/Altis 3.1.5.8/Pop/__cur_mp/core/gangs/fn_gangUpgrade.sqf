...
if(_action) then {
	if(ALUK_bankedwonga < _upgradePrice) exitWith {
		hint parseText format[
			...
			[ALUK_bankedwonga] call life_fnc_numberText,
			[_upgradePrice - ALUK_bankedwonga] call life_fnc_numberText
		];
	};
	__SUB__(ALUK_bankedwonga,_upgradePrice);
