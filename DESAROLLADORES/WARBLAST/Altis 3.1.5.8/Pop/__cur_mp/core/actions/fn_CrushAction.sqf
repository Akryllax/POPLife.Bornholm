...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	[[0,format["%1 your %2 is being scrapped by the police.",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	if(((count crew _vehicle) == 0) OR ((TypeOf _vehicle) == "B_UAV_01_F")) then
	{
		...
		hint format["You have crushed a %1\n\nYou have received 40XP!",_type];
		[[0,format["%1 has sent %2's %3 to the crusher!",name player,(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		ALUK_bankedwonga = ALUK_bankedwonga + _price;
		["crushing",20] call life_fnc_addProfessionPoints;
	}
		else
	{
