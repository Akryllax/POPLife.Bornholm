...
while {life_carryWeight < life_maxWeight} do
{
	...
	_items = _boat getVariable ["items",[]];
	...
	if(_sum > 0) then { 
		if (([true,_randomItem,1] call life_fnc_handleInv)) then {
			...
			//Subtract that (or one of similar)
			_index = _items find _randomItemLong;
			if(_index != -1) then {
				_items set[_index,"removeme"];
				_items = _items - ["removeme"];
			};
			_boat setVariable ["items",_items,true];
