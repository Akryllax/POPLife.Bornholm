...
if(life_disablehud) then {
	...
	_objs buttonSetAction "life_disablehud = false; [] call life_fnc_hudUpdate; [] call life_fnc_settingsMenu;";
} else {
	_objs ctrlSetTextColor [0,1,0,1];
	_objs ctrlSetText "Enabled";
	_objs buttonSetAction "life_disablehud = true; [] call life_fnc_hudUpdate; [] call life_fnc_settingsMenu;";
};
...
if(!(life_lowFPS)) then {
	...
	_fpsMode buttonSetAction "life_lowFPS = true; [] call life_fnc_changeFPS; [] call life_fnc_settingsMenu;";
	_fpsMode ctrlSetText "OFF";
} else {
	_fpsMode ctrlSetTextColor [0,1,0,1];
	_fpsMode buttonSetAction "life_lowFPS = false; [] call life_fnc_changeFPS; [] call life_fnc_settingsMenu;";
