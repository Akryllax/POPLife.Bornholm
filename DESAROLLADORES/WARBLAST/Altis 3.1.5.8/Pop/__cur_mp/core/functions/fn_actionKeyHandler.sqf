...
if(_curTarget isKindOf "Man" && {!alive _curTarget} && {playerSide in [west,independent]}) exitWith {
	//Hotfix code by ins0
	if(((playerSide == blufor && {side _x == independent} count playableUnits < 1) || playerSide == independent) && {"Medikit" in (items player)}) then {
		...
		if((typeOf _curTarget) in _animalTypes) then {
			...
			if((typeOf _curTarget) in _miscItems) then {
				...
				if((typeOf _curTarget) == _money && {!(_curTarget getVariable["inUse",false])}) then {
					...
					if((typeOf _curTarget) in (call life_placeables_classes) && playerSide in [west,independent]) then {
