...
if(ALUK_wonga >= _cost) then
{
	...
	[40] call life_fnc_addXP;
	//now see if we won
	if(_winAmount > 0) then
	{
		ALUK_wonga = ALUK_wonga + _winAmount;
...
{
	if(ALUK_wonga < _cost) exitWith {hint (format["You need £%1 to play this machine.",_cost]); 
	life_is_processing = false;};
};
