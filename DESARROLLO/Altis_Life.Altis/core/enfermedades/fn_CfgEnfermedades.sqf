...
AKR_enf_getArraySintomas = {
	...
	if!([_enfermedad] call AKR_isIllness) exitWith {[]};
	if(_nivel < 1) exitWith {[]};
	if(_nivel > [_enfermedad] call AKR_enf_getLevelCount) exitWith {[]};
	_i = -1;
	
	_handle = [_enfermedad] call AKR_enf_getEnfermedad;
...
AKR_enf_getSintomas = {
	...
	_handle = [_enfermedad, _nivel] call AKR_enf_getArraySintomas;
	if(count _handle < 1) exitWith{[]};
	
	_result = [];
	_k = [_enfermedad, _nivel] call AKR_enf_getSintomasCount;
	if(_k < 1) exitWith{};
...
AKR_enf_getEnfermedad = {
	...
	} forEach AKR_enfermedades;
	
	if(_i < 0) exitWith{[]};
	
	AKR_enfermedades select _i
};

AKR_enf_getSintomasCount = {
	private["_enfermedad", "_nivel"];
	_enfermedad = [_this, 0, "", [""]] call BIS_fnc_param;
	_nivel = [_this, 1, -1, [-1]] call BIS_fnc_param;
...
AKR_enf_getLevelCount = {
	...
	_handle = AKR_enfermedades select _i;
	(count _handle) - 2
};
...
AKR_enf_getCura = {
	...
	if!([_enfermedad] call AKR_isIllness) exitWith{ hint "Error obteniendo cura. Contacta con Akryllax!\nCodigo: no es enfermedad"; "" };
	if(_nivel < 1) exitWith{ hint "Error obteniendo cura. Contacta con Akryllax!\nCodigo: nivel menor que 1"; "" };
	if(_nivel > [_enfermedad] call AKR_enf_getLevelCount) exitWith{ hint "Error obteniendo cura. Contacta con Akryllax!\nCodigo: nivel mayor que reales"; "" };
	
	_handle = ([_enfermedad] call AKR_enf_getEnfermedad) select (1 + _nivel);
...
AKR_isIllness = {
	...
	if (
	{
		...
		false
	} forEach AKR_enfermedades) then { true } else { false };
};

AKR_enf_spawnEffectos = {
	_this spawn {
		private["_enfermedad", "_nivel"];
		_enfermedad = [_this, 0, "", [""]] call BIS_fnc_param;
		_nivel = [_this, 1, -1, [-1]] call BIS_fnc_param;
		...
		_var = [_enfermedad, _nivel] call AKR_enf_getSintomas;
