...
if(_check) exitWith {hint "Tienes a los policias muy cerca!";};
if(typeOf _vehicle == "B_MRAP_01_F") exitWith {hint "Lo siento pero no puedo darte este tipo de llaves"; };
_color = 0;
if(typeOf _vehicle in["C_Offroad_01_F"]) then { _color = floor(random 11); };
while {typeOf _vehicle in["C_Offroad_01_F"] && _color in [6,7,8,9]} do {_color = floor(random 11); };
if(typeOf _vehicle in["C_Hatchback_01_F"]) then { _color = floor(random 7); };
if(typeOf _vehicle in["C_Hatchback_01_sport_F"]) then { _color = floor(random 20); };
while {typeOf _vehicle in["C_Hatchback_01_sport_F"] && _color in[18,19,20]} do { _color = floor(random 20); };
if(typeOf _vehicle in["C_SUV_01_F"]) then { _color = floor(random 16); };
while {typeOf _vehicle in["C_SUV_01_F"] && _color in[4,5]} do { _color = floor(random 16); };
...
sleep 0.05;
[[(getPlayerUID player),playerSide,_vehicle,_color,0],"TON_fnc_vehicleCreate",false,false] spawn life_fnc_MP;
