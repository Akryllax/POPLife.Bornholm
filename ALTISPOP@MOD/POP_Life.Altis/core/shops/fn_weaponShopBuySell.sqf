...
if((life_inv_cb == 0) && (life_cash < _price)) exitWith {hint "No tiene suficiente dinero en efectivo y no tiene tarjeta de credito !"};
...
{
if((life_inv_cb == 0) && (life_cash < _price)) exitWith {hint "No tiene suficiente dinero en efectivo y no tiene una tarjeta de credito !"};
if((_gangbank >= _prixcb) && (life_inv_cb > 0)) then {
    ...
    "Mi dinero"
        ...
        _owner = (grpPlayer getVariable "gang_owner");
            ...
            [[0,format["Un arma %1 fue comprado por %3 por %2$ Con el dinero de la mafia",_itemInfo select 1,[_prixcb] call life_fnc_numberText,name player]],"life_fnc_broadcast",_owner,false] spawn life_fnc_MP;
        ...
        if(_action) then {
                ...
                if(_price > life_cash) exitWith {hint "No tiene suficiente dinero en efectivo !"};
                life_cash = life_cash - _price;
        ...
        if((life_inv_cb == 0)) then {
                if(_price > life_cash) exitWith {hint "no tiene suficiente dinero en efectivo y no dispone de una tarjeta de credito !"};
                life_cash = life_cash - _price;
                hint parseText format["Compro un %1 por <t color='#8cff9b'>%2$</t>",_itemInfo select 1,[_price] call life_fnc_numberText];
                [_item,true] spawn life_fnc_handleItem;
        } else {
