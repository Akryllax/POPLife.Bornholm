		...
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1) then
		{
			...
			{	
				...
				if("ItemRadio" in assignedItems cursorTarget) then {
					...
					hint "El telefono esta en el suelo.";
					_defenceplace1 = "Item_ItemRadio" createVehicle (player modelToWorld[0,0,0]);}
				else { hint "Esta persona no tiene telefono!"};
			};
		};
	};
		...
		case 24:
		{
		  ...
		  if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,independent,west]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		  {
		   if([false,"zipties",1] call life_fnc_handleInv) then
			{
			[] call life_fnc_restrainAction;
			hint "Utiliza boton windows para mas opciones";
		   }
		   else
		   {
...
if (_code in (actionKeys "PushToTalk") || _code in (actionKeys "PushToTalkSide")) then
{
	if ((call life_adminlevel) < 1) then
	{
		...
		if (_chan == "Side Channel") then {
			hint format["<t size='2' color='#0000FF'>No se puede hablar por el SIDE</t>", _chan];	_handled = true;
			player say3D "SysAlarm";
