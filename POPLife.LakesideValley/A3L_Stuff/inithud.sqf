while {true} do {
	...
	_playerhunger  = profileNamespace getVariable "hearb";
	_playerthirst  = profileNamespace getVariable "fois";
	...
	_gunmode       = "";
	if ((_gunmodeold == "single")or (_gunmodeold == "FullAuto") or (_gunmodeold == "Burst")) then {
			_gunmode = _gunmodeold;
			if (_gunmodeold == "single") then {_gunmode = "Semi-Auto"};
            if (_gunmodeold == "Burst") then {_gundmode = "Rafaga"};
			if (_gunmodeold == "FullAuto") then {_gunmode = "Automatica"};
		};
	...
	_mochila = life_carryWeight;
	_mochilapeso = life_maxWeight;
	_noweaponsarray = ["", "", "", "", 0];
	_nothing	= _noweaponsarray select 0;
	if (_nothing == _curweapon) then {} else {
	...
	if (_playerhunger > 90) then {
			...
			if (_playerhunger > 80) then {
					...
					if (_playerhunger > 70) then {
							_hungercolor = "#7BFF00";
							_hungertext  = parseText format ["Te vendrían bien algunas galletas"];
						} else {
							if (_playerhunger > 60) then {
									...
									if (_playerhunger > 50) then {
											_hungercolor = "#EAFF00";
											_hungertext  = parseText format ["Sientes el estomago algo vacío"];
										} else {
											if (_playerhunger > 40) then {
													_hungercolor = "#FFBF00";
													_hungertext  = parseText format ["Deberías comer pronto"];
												} else {
													if (_playerhunger > 30) then {
															_hungercolor = "#FF8400";
															_hungertext  = parseText format ["Tu estomago está vacío"];
														} else {
	...
	if (_playerthirst > 80) then {
			...
			if (_playerthirst > 70) then {
					_thirstcolor = "#7BFF00";
					_thirsttext  = parseText format ["Te vendría bien un vaso de agua"];
				} else {
					if (_playerthirst > 60) then {
							_thirstcolor = "#BFFF00";
							_thirsttext  = parseText format ["Deberías beber algo pronto"];
						} else {
							if (_playerthirst > 50) then {
									_thirstcolor = "#EAFF00";
									_thirsttext  = parseText format ["Tu garganta está algo seca"];
								} else {
									if (_playerthirst > 40) then {
											...
											if (_playerthirst > 30) then {
													_thirstcolor = "#FF8400";
													_thirsttext  = parseText format ["Tu garganta está seca"];
												} else {
													if (_playerthirst > 20) then {
															_thirstcolor = "#FF4000";
															_thirsttext  = parseText format ["Deberías beber ahora mismo"];
														} else {
	...
	if (_playerstamina < 0.10) then {
			...
			if (_playerstamina < 0.20) then {
					...
					_staminatext  = parseText format ["Podría correr siempre"];
				} else {
					if (_playerstamina < 0.30) then {
							_staminacolor = "#BFFF00";
							_staminatext  = parseText format ["Empiezas a sentirte cansado"];
						} else {
							if (_playerstamina < 0.40) then {
									...
									if (_playerstamina < 0.50) then {
											...
											if (_playerstamina < 0.60) then {
													...
													_staminatext  = parseText format ["Estás empezando a agotarte"];
												} else {
													if (_playerstamina > 0.59) then {
															_staminacolor = "#590000";
															_staminatext  = parseText format ["Estás agotado"];
														};
												};
										};
								};
	...
	_playernametext	  = parseText format ["<t font='EtelkaNarrowMediumPro' color='#FFFFFF' size='0.9' align='right'>%1 | FPS %2 | EFECTIVO %3 | PESO %4/%5 </t>", _nameofplayer,round diag_fps, [life_cash] call life_fnc_numberText,_mochila,_mochilapeso];
