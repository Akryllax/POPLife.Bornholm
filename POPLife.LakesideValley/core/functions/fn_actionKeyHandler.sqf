...
*/
private["_curTarget","_isWater","_veh","_animal"];
...
if (typeOf _curTarget IN ["A3L_Wheat","A3L_Corn","A3L_Beans","A3L_Cannabis","A3L_Cotton","Ficus_Bush_1","A3L_Pumpkin","A3L_Sunflower","Oleander2","Carduus","Flower_Cakile","cl_mushroom","cl_coca_plant"]) then {
	// It's a plant!
		...
		if((typeOf _curTarget) in _animalTypes) then {
			...
			if((typeOf _curTarget) in _miscItems) then {
				...
				if ((typeOf _curTarget) == "Land_Suitcase_F") then {
					...
					if (_var) then {
						if (count (attachedobjects player) > 0) exitwith {["Usted ya esta llevando una maleta.",30,"red"] call A3L_Fnc_Msg;}; 
						if (side player == west) then {
								["Traiga esta maleta a la DP de una recompensa!",30,"blue"] call A3L_Fnc_Msg;
								...
								_id = player addAction ["Tirar Maleta", {detach (_this select 3)},_curTarget];
						...
						if (side player == civilian) then {
								["Traiga esta maleta a un comerciante para obtener el efectivo!",30,"blue"] call A3L_Fnc_Msg;
								_curTarget setVariable ["pickup",false,true];
								_curTarget attachto [player, [0.035,-.055,-0.22], "RightHandMiddle1"];
								_curTarget setdir (getdir player + 180);
								_curTarget setpos (getpos _veh);
