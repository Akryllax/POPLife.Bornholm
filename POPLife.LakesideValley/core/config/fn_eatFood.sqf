...
if([false,_food,1] call life_fnc_handleInv) then {
	...
	_sum = (profileNamespace getVariable "hearb") + _val;
	if(_sum > 100) then {_sum = 100; player setFatigue 1; hint "Has comido demasiado... Ahora tu fatiga aumentara.";};
	profileNamespace setVariable ["hearb",_sum];
};