...
if((lbCurSel 2302) == -1) exitWith {hint "No ha seleccionado dingun vehiculo !"};
...
if((life_inv_cb == 0) && (life_cash < _basePrice)) exitWith {hint "no tiene suficiente dinero en efectivo y no tiene tarjeta de credito !"};
...
if((_gangbank >= _prixcb) && (life_inv_cb > 0)) then {
        _action = [
            format["DINERO DE LA MAFIA : <t color='#8cff9b'>%1$</t><br/>CUENTA BANCARIA : <t color='#8cff9b'>%5$</t><br/>EFECTIVO : <t color='#8cff9b'>%2$</t><br/><br/>Efectivo : <t color='#8cff9b'>%3$</t><br/>Tarjeta : <t color='#8cff9b'>%4$</t>",
    ...
    "Mi dinero"
        ...
        if(_action) then {
            ...
            [[0,format["El vehiculo %1 fue comprado por %3 por el Precio %2$ Con el dinero de la Mafia",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_prixcb] call life_fnc_numberText,name player]],"life_fnc_broadcast",_owner,false] spawn life_fnc_MP;
            ...
            if(_action) then {
                    if(_prixcb > life_atmcash) exitWith {hint "no tiene suficiente dinero en su cuenta !"};
                    life_atmcash = life_atmcash - _prixcb;
                    _secure = 1;                    
            } else {
                    if(_basePrice > life_cash) exitWith {hint "no tiene suficiente dinero en efectivo !"};
