...
if (side player == west) then {
		tabletapp_array = [ 
					["Menu", "menu", "\A3L_Client2\tablet_new\icons\playermenu.paa", "\A3L_Client2\tablet_new\icons\clicked\playermenu.paa", [""], "playerSettings"],
				    ["LLaves", "Llaves", "\A3L_Client2\tablet_new\icons\keys.paa", "\A3L_Client2\tablet_new\icons\clicked\keys.paa", [""], "Life_key_management"],
				    ["Configuracion", "Ajustes", "\A3L_Client2\tablet_new\icons\settings.paa", "\A3L_Client2\tablet_new\icons\clicked\settings.paa", [""], "SettingsMenu"],
				    ["Sincronizar", "Sincronizar", "\A3L_Client2\tablet_new\icons\sync.paa", "\A3L_Client2\tablet_new\icons\clicked\sync.paa", [""], "SyncMenu"],
				    ["Movil", "Movil", "\A3L_Client2\tablet_new\icons\messanger.paa", "\A3L_Client2\tablet_new\icons\clicked\messanger.paa", [""], "Life_cell_phone"],
				    ["Buscados", "Buscados", "\A3L_Client2\tablet_new\icons\wanted.paa", "\A3L_Client2\tablet_new\icons\clicked\wanted.paa", [""], "life_wanted_menu"],
					["Admin", "Admin", "\A3L_Client2\tablet_new\icons\adminp.paa", "\A3L_Client2\tablet_new\icons\adminp.paa", [""], "life_admin_menu"]
		];
	} else {
		tabletapp_array = [ 
					["Menu", "menu", "\A3L_Client2\tablet_new\icons\playermenu.paa", "\A3L_Client2\tablet_new\icons\clicked\playermenu.paa", [""], "playerSettings"],
				    ["Llaves", "Llaves", "\A3L_Client2\tablet_new\icons\keys.paa", "\A3L_Client2\tablet_new\icons\clicked\keys.paa", [""], "Life_key_management"],
				    ["Configuracion", "Configuracion", "\A3L_Client2\tablet_new\icons\settings.paa", "\A3L_Client2\tablet_new\icons\clicked\settings.paa", [""], "SettingsMenu"],
				    ["Sincronizar", "Sincronizar", "\A3L_Client2\tablet_new\icons\sync.paa", "\A3L_Client2\tablet_new\icons\clicked\sync.paa", [""], "SyncMenu"],
				    ["Movil", "Movil", "\A3L_Client2\tablet_new\icons\messanger.paa", "\A3L_Client2\tablet_new\icons\clicked\messanger.paa", [""], "Life_cell_phone"],
					["Movil", "Movil", "\A3L_Client2\tablet_new\icons\messanger.paa", "\A3L_Client2\tablet_new\icons\clicked\messanger.paa", [""], "Life_cell_phone"],
					["Admin", "Admin", "\A3L_Client2\tablet_new\icons\adminp.paa", "\A3L_Client2\tablet_new\icons\adminp.paa", [""], "life_admin_menu"],
					["Mafias", "mafias", "\A3L_Client2\tablet\icons\cpanel.paa", "\A3L_Client2\tablet\icons\cpanel.paa", [""], "if(isNil ""life_action_gangInUse"") then {if(isNil {(group player) getVariable ""gang_owner""}) then {createDialog ""Life_Create_Gang_Diag"";} else {[] spawn life_fnc_gangMenu;};};"],
					["Crafteo", "Crafteo", "\A3L_Client2\tablet_new\icons\playermenu.paa", "\A3L_Client2\tablet_new\icons\playermenu.paa", [""], "['menu'] execVM 'ica\jugador\crafteo.sqf'"],
					["Experiencia", "Experiencia", "\A3L_Client2\tablet\icons\cpanel.paa", "\A3L_Client2\tablet\icons\cpanel.paa", [""], "['menu'] execVM 'ica\exp\exp.sqf'"],
					["Baile", "Baile", "\A3L_Client2\tablet_new\icons\playermenu.paa", "\A3L_Client2\tablet_new\icons\playermenu.paa", [""], "createDialog ""animaBaile_dialog"]
...
fnc_opentablet = {
	...
	_monthtext = 0;
	_daytext   = 0;
	if (_month == 1) then {_monthtext = "Enero";
		};
	if (_month == 2) then {_monthtext = "Febrero";
		};
	if (_month == 3) then {_monthtext = "Marzo";
		};
	if (_month == 4) then {_monthtext = "Abril";
		};
	if (_month == 5) then {_monthtext = "Mayo";
		};
	if (_month == 6) then {_monthtext = "Junio";
		};
	if (_month == 7) then {_monthtext = "Julio";
		};
	if (_month == 8) then {_monthtext = "Agosto";
		};
	if (_month == 9) then {_monthtext = "Septiembre";
		};
	if (_month == 10) then {_monthtext = "Octubre";
		};
	if (_month == 11) then {_monthtext = "Noviembre";
		};
	if (_month == 12) then {_monthtext = "Diciembre";
		};
	if (_day == 1) then {_daytext =  parseText format ["%1st", _day];
		} else                                                           {
			if (_day == 2) then {_daytext =  parseText format ["%1nd", _day];
				} else                                                           {
					if (_day == 3) then {_daytext =  parseText format ["%1rd", _day];
						} else                                                           {
	...
	_windDir = 0;
	_weather = "NO SIGNAL";
	if (windDir >= 340 && windDir <= 360 || windDir >= 0 && windDir <= 20) then {_windDir = "Norte";
		};
	if (windDir >= 20 && windDir <= 60) then {_windDir = "Nor-Este";
		};
	if (windDir >= 60 && windDir <= 110) then {_windDir = "Este";
		};
	if (windDir >= 110 && windDir <= 160) then {_windDir = "Sur-Este";
		};
	if (windDir >= 160 && windDir <= 200) then {_windDir = "Sur";
		};
	if (windDir >= 200 && windDir <= 250) then {_windDir = "Sur-Oeste";
		};
	if (windDir >= 250 && windDir <= 290) then {_windDir = "Oeste";
		};
	if (windDir >= 290 && windDir <= 340) then {_windDir = "Nor-Oeste";
		};

	if (rain < 0.05) then {
			if (overcast >= 0 && overcast < 0.4) then {_weather = "Despejado";
				} else                                                        {
					if (overcast >= 0.4 && overcast < 0.6) then {_weather = "Nuboso";
						} else                                                           {
							if (overcast >= 0.6 && overcast <= 1) then {_weather = "Niebla";
								};
						};
				};
		} else {
			if (overcast >= 0 && overcast < 0.6) then {_weather = "Lluvia";
				} else                                                       {
					if (overcast >= 0.6 && overcast <= 1) then {_weather = "Tormenta";
						};
				};
		};
	...
	_weatherinfo = parseText format ["<t font='EtelkaNarrowMediumPro' color='#FFFFFF' align='left' size='0.6'>Dirección del viento:  %1.</t>", _windDir];
	_graden	     = parseText format ["<t font='EtelkaNarrowMediumPro' color='#FFFFFF' align='left'><t size='2.5'>6 °C</t></t>"];
