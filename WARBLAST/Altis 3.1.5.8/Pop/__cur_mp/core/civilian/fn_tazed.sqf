...
if(_shooter isKindOf "Man" && alive player) then
{
	if((typeName life_istazed != typeName false) || {!life_istazed}) then
	{
		...
		if(side _shooter == civilian) then {
			[[0,format[localize "STR_NOTF_Tazed", _unit getVariable["realname",name _unit], format["%1 (BountyHunter)",_shooter getVariable["realname",name _shooter]]]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		} else {
