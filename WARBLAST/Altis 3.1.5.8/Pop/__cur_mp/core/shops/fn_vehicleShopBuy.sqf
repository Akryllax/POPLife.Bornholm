...
//_mode = _this select 0; //Sure Tonic's got this wrong, so rewriting to use bis_fnc_param...
...
if(!(_mode)) then {_basePrice = round(_basePrice * 0.5)}; //NDJ rental vehicle, half price!
_colorIndex = lbValue[2304,(lbCurSel 2304)];

//Series of checks (YAY!)
if(_basePrice < 0) exitWith {}; //Bad price entry
if(ALUK_wonga < _basePrice) exitWith {hint format[localize "STR_Shop_Veh_NotEnough",[_basePrice - ALUK_wonga] call life_fnc_numberText];};
...
ALUK_wonga = ALUK_wonga - _basePrice;
playSound "buy";
...
if(!(_className in ["B_UAV_01_F","B_Quadbike_01_F","B_UGV_01_rcws_F"])) then {
...
if(_className == "C_SUV_01_F") then {
	//Default from getCentreOfMass: [-0.010813,-0.506166,-0.557481]
	_vehicle setCenterOfMass [-0.010813,-0.506166,-0.7];
};
