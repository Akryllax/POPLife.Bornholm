...
if(isServer) then {
	...
	{
		if(!isNull _x && isPlayer _x) then {[[3,_startengines],"life_fnc_broadcast",_x,false] spawn life_fnc_MP;}
	} forEach _near;
	...
	{
		if(!isNull _x && isPlayer _x) then {[[3,_begin],"life_fnc_broadcast",_x,false] spawn life_fnc_MP;}
	} forEach _near;
