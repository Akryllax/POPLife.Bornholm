	...
	if(_source != _unit) then {
		...
		if(_projectile in ["B_9x21_Ball","B_556x45_dual"] && _curWep in ["hgun_P07_snds_F","arifle_SDAR_F"]) then {
			if(side _source == west && playerSide != west) then {
				...
				_isQuad = if(_isVehicle) then {if(typeOf (vehicle player) == "B_Quadbike_01_F" || typeOf (vehicle player) == "C_Kart_01_Blu_F" || typeOf (vehicle player) == "C_Kart_01_Red_F" || typeOf (vehicle player) == "C_Kart_01_Fuel_F" || typeOf (vehicle player) == "C_Kart_01_Vrana_F") then {true} else {false}} else {false};
		...
		if(_projectile == "B_9x21_Ball" && _curWep == "hgun_Rook40_snds_F") then {
			if(side _source == west && playerSide != west) then {
				...
				_damage = false;
				_damageHandle = false;
				if(_unit distance _source < 70) then {
					if(!life_istazed && !(_unit getVariable["restrained",false])) then {
						if(_isVehicle && _isQuad) then {
							player action ["Eject",vehicle player];
							[_unit,_source] spawn life_fnc_tazed;
